        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-8">
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
                    <h2 class="text-xl font-black text-gray-900 mb-6 uppercase italic tracking-tighter pb-3 border-b-2 border-gray-50">Informations</h2>
                    <ul class="space-y-6">
                        <li>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-6 p-6 rounded-[1.5rem] bg-gray-50 border border-transparent hover:border-blue-100 hover:bg-white hover:shadow-lg transition-all group">
                            <div class="h-16 w-16 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center">
                                <span class="text-xs font-black text-gray-400 uppercase leading-none">{{ substr($daysMapping[$slot->day_of_week] ?? $slot->day_of_week, 0, 3) }}.</span>
                            </div>
                            
                            <div class="flex-1">
                                <h3 class="font-black text-gray-900 text-lg uppercase">{{ $daysMapping[$slot->day_of_week] ?? $slot->day_of_week }}</h3>
                            ...
                            <div class="text-right">
                                <div class="flex items-center justify-end gap-2 text-blue-600 font-black text-xl italic tracking-tighter mb-1">
                                    ...
                                    <span>{{ \Carbon\Carbon::parse($slot->end_time)->format('H:i') }}</span>
                                </div>
                                <span class="px-3 py-1 {{ $slot->is_active ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600' }} rounded-lg font-black text-[9px] uppercase tracking-widest">{{ $slot->is_active ? 'En Service' : 'Absent' }}</span>
                            </div>
                        </div>
                        @empty
                        <div class="text-center py-20 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200">
                                ...
                                <button class="mt-6 px-10 py-4 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:shadow-blue-200 transition-all">Initialiser mon Planning</button>
                        </div>
                        @endforelse
                    </div>
